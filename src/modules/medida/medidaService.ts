import type { LoginUserDto } from '../auth/authDto.js';
import { findCompanyById } from '../company/companyDao.js';
import { findUserById } from '../user/userDao.js';
import { logger } from '../../utils/logger.js';
import { validateRequiredString, validateStatus } from '../../utils/validation.js';
import type {
  CreateMedidaDto,
  FindMedidaDto,
  FindMedidasParamsDto,
  FindMedidasResponseDto,
  MedidaResponseDto,
  UpdateMedidaDto,
} from './medidaDto.js';
import {
  findMedidaByAbbreviation,
  findMedidaById,
  findMedidaByName,
  findMedidas,
  saveMedida,
  updateMedidaById,
} from './medidaDao.js';

const EMPTY_COMPANY_ID_MESSAGE = 'El id de empresa es requerido';
const EMPTY_MEDIDA_ID_MESSAGE = 'El id de medida es requerido';
const EMPTY_MEDIDA_NAME_MESSAGE = 'El nombre de medida es requerido';
const EMPTY_MEDIDA_ABBREVIATION_MESSAGE = 'La abreviatura de medida es requerida';
const EMPTY_MEDIDA_STATUS_MESSAGE = 'El estado de medida es requerido';
const INVALID_MEDIDA_UPDATE_STATUS_MESSAGE = 'El estado de medida debe ser activo, inactivo o eliminado';
const INVALID_COMPANY_FIND_MESSAGE = 'La empresa no existe';
const INVALID_COMPANY_STATUS_MESSAGE = 'La empresa no esta activa';
const INVALID_USER_NOT_FOUND_MESSAGE = 'El usuario no existe';
const INVALID_USER_NOT_BELONG_COMPANY_MESSAGE = 'El usuario no pertenece a la empresa';
const INVALID_USER_STATUS_MESSAGE = 'El usuario no esta activo';
const FORBIDDEN_ROL_USER_ADMIN_MESSAGE = 'El usuario no es administrador';
const FORBIDDEN_ROL_USER_MESSAGE = 'El usuario no es jefe, empleado o administrador';
const FORBIDDEN_ROL_USER_JEFE_OR_EMPLEADO_MESSAGE = 'El usuario no es jefe o empleado';
const FORBIDDEN_COMPANY_CREATION_MESSAGE = 'La empresa no es empresa padre';
const FORBIDDEN_CROSS_COMPANY_ACCESS_MESSAGE = 'El usuario no puede acceder a otra empresa';
const INVALID_PAGE_MESSAGE = 'La pagina debe ser un entero positivo';
const INVALID_PAGE_SIZE_MESSAGE = 'El tamano de pagina debe ser un entero positivo';
const INVALID_MEDIDA_EXISTS_MESSAGE = 'Ya existe una medida con ese nombre';
const INVALID_MEDIDA_ABBREVIATION_EXISTS_MESSAGE = 'Ya existe una medida con esa abreviatura';
const EMPTY_UPDATE_MEDIDA_MESSAGE = 'Al menos un campo es requerido para actualizar la medida';
const FORBIDDEN_UPDATE_DELETED_MEDIDA_MESSAGE = 'La medida eliminada no puede ser actualizada';

type AccessOptions = {
  requireParentCompany: boolean;
  requireAdminUser: boolean;
  targetCompanyId?: string;
};

function validateFindMedidasParams(params: FindMedidasParamsDto): FindMedidasParamsDto {
  const { page, pageSize, search, status } = params;

  if (!Number.isInteger(page) || page < 1) {
    throw new Error(INVALID_PAGE_MESSAGE);
  }

  if (!Number.isInteger(pageSize) || pageSize < 1) {
    throw new Error(INVALID_PAGE_SIZE_MESSAGE);
  }

  const normalizedSearch = typeof search === 'string'
    ? search.trim()
    : undefined;

  const validatedParams: FindMedidasParamsDto = {
    page,
    pageSize,
  };

  if (normalizedSearch && normalizedSearch.length > 0) {
    validatedParams.search = normalizedSearch;
  }

  if (status) {
    validatedParams.status = status;
  }

  return validatedParams;
}

async function validateCompanyAndUserAccess(user: LoginUserDto, options: AccessOptions): Promise<void> {
  const { requireParentCompany, requireAdminUser, targetCompanyId } = options;

  const companyDB = await findCompanyById(user.usemid);
  if (!companyDB) {
    throw new Error(INVALID_COMPANY_FIND_MESSAGE);
  }

  const isActiveCompany = companyDB.emestado;
  if (isActiveCompany !== 'activo') {
    throw new Error(INVALID_COMPANY_STATUS_MESSAGE);
  }

  const userDB = await findUserById({
    usid: user.usid,
    usemid: user.usemid,
  });

  if (!userDB) {
    throw new Error(INVALID_USER_NOT_FOUND_MESSAGE);
  }

  const isUserCompany = userDB.usemid;
  if (isUserCompany !== user.usemid) {
    throw new Error(INVALID_USER_NOT_BELONG_COMPANY_MESSAGE);
  }

  if (targetCompanyId && targetCompanyId !== user.usemid) {
    throw new Error(FORBIDDEN_CROSS_COMPANY_ACCESS_MESSAGE);
  }

  if (requireAdminUser && userDB.usrol !== 'administrador') {
    throw new Error(FORBIDDEN_ROL_USER_ADMIN_MESSAGE);
  }

  if (!requireAdminUser && !['jefe', 'empleado', 'administrador'].includes(userDB.usrol)) {
    throw new Error(FORBIDDEN_ROL_USER_MESSAGE);
  }

  const isActiveUser = userDB.usestado;
  if (isActiveUser !== 'activo') {
    throw new Error(INVALID_USER_STATUS_MESSAGE);
  }

  if (requireParentCompany && !companyDB.empadre) {
    throw new Error(FORBIDDEN_COMPANY_CREATION_MESSAGE);
  }
}

async function validateRequesterJefeOrEmpleado(user: LoginUserDto): Promise<void> {
  const requesterUser = await findUserById({
    usid: user.usid,
    usemid: user.usemid,
  });

  if (!requesterUser) {
    throw new Error(INVALID_USER_NOT_FOUND_MESSAGE);
  }

  if (!['jefe', 'empleado'].includes(requesterUser.usrol)) {
    throw new Error(FORBIDDEN_ROL_USER_JEFE_OR_EMPLEADO_MESSAGE);
  }
}

async function createMedida(medida: CreateMedidaDto, user: LoginUserDto): Promise<MedidaResponseDto> {
  const mdiaemid = validateRequiredString(medida.mdiaemid, EMPTY_COMPANY_ID_MESSAGE);
  const mdianombre = validateRequiredString(medida.mdianombre, EMPTY_MEDIDA_NAME_MESSAGE);
  const mdiaabreviatura = validateRequiredString(medida.mdiaabreviatura, EMPTY_MEDIDA_ABBREVIATION_MESSAGE);

  try {
    const access = {
      requireParentCompany: false,
      requireAdminUser: false,
      targetCompanyId: mdiaemid,
    };

    await validateCompanyAndUserAccess(user, access);
    await validateRequesterJefeOrEmpleado(user);

    const medidaByNameDB = await findMedidaByName({mdiaemid, mdianombre});

    if (medidaByNameDB) {
      throw new Error(INVALID_MEDIDA_EXISTS_MESSAGE);
    }

    const medidaByAbbreviationDB = await findMedidaByAbbreviation({mdiaemid, mdiaabreviatura});

    if (medidaByAbbreviationDB) {
      throw new Error(INVALID_MEDIDA_ABBREVIATION_EXISTS_MESSAGE);
    }

    const medidaId = await saveMedida({
      mdiaemid,
      mdianombre,
      mdiaabreviatura,
    });

    const newMedida = await findMedidaById({mdiaemid, mdiaid: medidaId});

    return newMedida!;

  } catch (error) {
    logger.error(
      {
        err: error,
        companyId: mdiaemid,
        medidaName: mdianombre,
        medidaAbbreviation: mdiaabreviatura,
        requesterUserId: user.usid,
        requesterCompanyId: user.usemid,
      },
      'Error creating medida',
    );
    throw error;
  }
}

async function readMedida(medida: FindMedidaDto, user: LoginUserDto): Promise<MedidaResponseDto | null> {
  const mdiaid = validateRequiredString(medida.mdiaid, EMPTY_MEDIDA_ID_MESSAGE);

  try {
    const access = {
      requireParentCompany: false,
      requireAdminUser: false,
      targetCompanyId: user.usemid,
    };

    await validateCompanyAndUserAccess(user, access);
    await validateRequesterJefeOrEmpleado(user);

    const medidaDB = await findMedidaById({
      mdiaemid: user.usemid,
      mdiaid,
    });

    if (!medidaDB) {
      throw new Error('Medida no encontrada');
    }

    return medidaDB;
  } catch (error) {
    logger.error(
      {
        err: error,
        medidaId: mdiaid,
        requesterUserId: user.usid,
        requesterCompanyId: user.usemid,
      },
      'Error reading medida',
    );
    throw error;
  }
}

async function readMedidas(
  params: FindMedidasParamsDto,
  user: LoginUserDto,
): Promise<FindMedidasResponseDto> {
  const validatedParams = validateFindMedidasParams(params);

  try {
    const access = {
      requireParentCompany: false,
      requireAdminUser: false,
      targetCompanyId: user.usemid,
    };

    await validateCompanyAndUserAccess(user, access);
    await validateRequesterJefeOrEmpleado(user);

    const medidasDB = await findMedidas(validatedParams, user.usemid);

    return medidasDB;
  } catch (error) {
    logger.error(
      {
        err: error,
        page: validatedParams.page,
        pageSize: validatedParams.pageSize,
        requesterUserId: user.usid,
        requesterCompanyId: user.usemid,
      },
      'Error reading medidas',
    );
    throw error;
  }
}

async function updateMedida(medida: UpdateMedidaDto, user: LoginUserDto): Promise<MedidaResponseDto | null> {
  const mdiaid = validateRequiredString(medida.mdiaid, EMPTY_MEDIDA_ID_MESSAGE);
  const mdianombre = medida.mdianombre !== undefined
    ? validateRequiredString(medida.mdianombre, EMPTY_MEDIDA_NAME_MESSAGE)
    : undefined;
  const mdiaabreviatura = medida.mdiaabreviatura !== undefined
    ? validateRequiredString(medida.mdiaabreviatura, EMPTY_MEDIDA_ABBREVIATION_MESSAGE)
    : undefined;
  const mdiaestado = medida.mdiaestado !== undefined
    ? validateStatus(medida.mdiaestado, EMPTY_MEDIDA_STATUS_MESSAGE, INVALID_MEDIDA_UPDATE_STATUS_MESSAGE)
    : undefined;

  try {
    const access = {
      requireParentCompany: false,
      requireAdminUser: false,
      targetCompanyId: user.usemid,
    };

    await validateCompanyAndUserAccess(user, access);
    await validateRequesterJefeOrEmpleado(user);

    const medidaDB = await findMedidaById({
      mdiaemid: user.usemid,
      mdiaid,
    });

    if (!medidaDB) {
      return null;
    }

    if (medidaDB.mdiaestado === 'eliminado') {
      throw new Error(FORBIDDEN_UPDATE_DELETED_MEDIDA_MESSAGE);
    }

    if (mdianombre !== undefined) {
      const medidaByNameDB = await findMedidaByName({
        mdiaemid: user.usemid,
        mdianombre,
      });

      if (medidaByNameDB && medidaByNameDB.mdiaid !== mdiaid) {
        throw new Error(INVALID_MEDIDA_EXISTS_MESSAGE);
      }
    }

    if (mdiaabreviatura !== undefined) {
      const medidaByAbbreviationDB = await findMedidaByAbbreviation({
        mdiaemid: user.usemid,
        mdiaabreviatura,
      });

      if (medidaByAbbreviationDB && medidaByAbbreviationDB.mdiaid !== mdiaid) {
        throw new Error(INVALID_MEDIDA_ABBREVIATION_EXISTS_MESSAGE);
      }
    }

    const dataDB: {
      column: string;
      value: string | number | boolean | Date | null;
    }[] = [];

    if (mdianombre !== undefined && mdianombre !== medidaDB.mdianombre) {
      dataDB.push({ column: 'mdianombre', value: mdianombre });
    }

    if (mdiaabreviatura !== undefined && mdiaabreviatura !== medidaDB.mdiaabreviatura) {
      dataDB.push({ column: 'mdiaabreviatura', value: mdiaabreviatura });
    }

    if (mdiaestado !== undefined && mdiaestado !== medidaDB.mdiaestado) {
      dataDB.push({ column: 'mdiaestado', value: mdiaestado });
    }

    if (dataDB.length === 0) {
      throw new Error(EMPTY_UPDATE_MEDIDA_MESSAGE);
    }

    const updatedMedidaDB = await updateMedidaById(dataDB, {
      mdiaemid: user.usemid,
      mdiaid,
    });

    if (!updatedMedidaDB) {
      return null;
    }

    return updatedMedidaDB;
  } catch (error) {
    logger.error(
      {
        err: error,
        medidaId: mdiaid,
        hasName: mdianombre !== undefined,
        hasAbbreviation: mdiaabreviatura !== undefined,
        hasStatus: mdiaestado !== undefined,
        requesterUserId: user.usid,
        requesterCompanyId: user.usemid,
      },
      'Error updating medida',
    );
    throw error;
  }
}

export { createMedida, readMedida, readMedidas, updateMedida };
